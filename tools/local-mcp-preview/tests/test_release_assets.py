...
class LocalMcpReleaseAssetTests(unittest.TestCase):
    def test_release_tag_must_bind_current_core_version(self) -> None:
        passing = subprocess.run(
            [sys.executable, str(TOOL), "validate-tag", "--tag", "v0.2.0-alpha.3"],
            cwd=REPO_ROOT,
            check=False,
            capture_output=True,
            text=True,
    ...
    def test_release_notes_name_both_exact_native_artifact_sets(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            self.assertIn("contextdb-local-mcp-0.2.0-alpha.3-linux-x86_64.zip", content)
            self.assertIn("contextdb-local-mcp-0.2.0-alpha.3-windows-x86_64.zip", content)
            self.assertIn("Ubuntu 22.04 LTS / glibc 2.35 or newer", content)
